...
function loadMore(res) {
    res.forEach(function (bot) {
        if (bot.state == "não verificado") return;
        ...
        <div class="card">
            ...
            <a href="/bots/${bot.botid}" class="button small">Ver info do bot</a>
        </div>`
...
function search() {
    ...
    if (cards) {
        ...
        if (cardsVisible === 0) {
            document.getElementById('searchMore').innerHTML = `Sem bot aki. tente procurar <a href="/bots/search/?q=${s}">procurar bots</a>?`;
            document.getElementById('searchMore').style.display = "block";
        } else {
            document.getElementById('searchMore').innerHTML = `tente procurar <a href="/bots/search/?q=${s}">procurar por bots</a>`
            document.getElementById('searchMore').style.display = "block";
        }
