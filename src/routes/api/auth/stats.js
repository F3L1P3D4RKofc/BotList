...
route.post('/:id', authApi, async (req, res) => {
    ...
    if (!count) return res.json({ success: "false", error: "Não encontrado no body." });
    ...
    if (bot.servers.length > 0 && bot.servers[bot.servers.length-1] &&  Date.now() - bot.servers[bot.servers.length-1].time < ratelimit * 1000) return res.json({ success: "false", error: "Tente novamente mais tarde." });
