...
route.post("/:id", auth, async (req, res) => {
    ...
    try {
        ...
        return res.json({ success: false, message: "Erro desconhecidp" })
    }
    let { bot, users } = check;
    let data = req.body;
    ...
    if (original && original.state !== "deletado")
        return res.json({ success: false, message: "Seu bot já foi adicionado no site..", button: { text: "Edit", url: `/bots/edit/${bot.id}` } });
    else if (original && original.state == "deletado") resubmit = true;
