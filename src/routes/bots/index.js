...
route.get('/:id', async (req, res) => {
    ...
    if (bot.state === "deletado") return res.render("404", {req})

    let owners = [bot.owners.primary].concat(bot.owners.additional);

    // If bot is unverified, check that the user is either a bot owner, admin or bot verifier
    if (bot.state == "não verificado" && (!req.user || !owners.includes(req.user.id) && !admin_user_ids.includes(req.user.id))) {
    ...
    try {
        ...
        owners = [{tag: "Usuário não encontrado"}]
    }
    let b = "#8c8c8c";
